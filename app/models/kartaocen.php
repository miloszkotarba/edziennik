...
class KartyOcen
{
    ...
    public function checkWychowawstwo($usersId)
    {
        $this->db->query('SELECT usersId, usersName, usersSurname, k.klasaName, k.klasaId FROM users u JOIN klasa k ON k.tutorId = u.usersId WHERE usersId = :usersId;');
    ...
    public function showUserFromClass($klasaId)
    {
        $this->db->query('SELECT usersId, usersName, usersSurname, (SELECT klasaName FROM klasa WHERE klasaId = :klasaId) klasa
FROM users WHERE klasaId = :klasaId ORDER BY 3,2;');
